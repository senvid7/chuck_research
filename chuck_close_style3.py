...
def draw_common(local_photo,max_color,sencond_color,third_color,fourth_color,cr,weight_l,height_k,beilv,svg_hist_size,stroke_size,type_h):
    ...
    rand_color,rand_second_color,out_third_color= find_outer_color(draw_line,svg_hist_size,sencond_color,stroke_size,type_h)
...
def find_outer_color(draw_line,svg_hist_size,second_color,stroke_size,type_h):
    rang = 40
    flag = 0
    path = 0
    path_2 = 0
    ...
    weight_2 = weight_2/(weight_2+weight_3)
    weight_3 = weight_3/(weight_2+weight_3)
    while(flag == 0 ):
        if(path < 40):
...
def svg_out_draw(rand_color,rand_second_color,out_third_color,weight_l,height_k,svg_hist_size,stroke_size,cr,type_h):  
...
def draw_hist(probs,bins,local_photo,num_color,center,draw_line,weight_l,height_k,cr,beilv,svg_hist_size,stroke_size,type_h):
    probs_list = sorted(probs)
    for m in range (0,num_color):
    ...
    if draw_line_second.shape[0] < 3 or compute_polygon_area(draw_line) < (n*m*beilv*beilv//14):
        local_photo = draw_common(local_photo,max_color,third_color,sencond_color,fourth_color,cr,weight_l,height_k,beilv,svg_hist_size,stroke_size,type_h)                
    else:
        if type_h == 1:
            weight_l = weight_l - (svg_hist_size-1)/2
            height_k = height_k - (svg_hist_size-1)/2
    ...
    while i < (m-half_size):
        ...
        while j < (n-half_size):
             ...
             if (bins[4] - bins[0]) < color_size :
                 ...
                 cr.save()
                 local_photo1 = draw_common(local_photo,max_color,third_color,second_color,fourth_color,cr,weight_l,height_k,beilv,svg_hist_size,stroke_size,type_h)  
                 cr.restore()
             else:
                 ...
                 if draw_line is None :
                     max_color,second_color,third_color,fourth_color = find_background_color(local_photo,hist_size)
                     cr.save()
                     local_photo1 = draw_common(local_photo_m,max_color,third_color,second_color,fourth_color,cr,weight_l,height_k,beilv,svg_hist_size,stroke_size,type_h)  
                     cr.restore()
                 else:
                     draw_line = Interpolation(draw_line,71)
                     draw_line = remove_same_list(draw_line)
                     ...
                     cr.save()
                     local_photo1 = draw_hist(probs,bins,local_photo_m,num_color,center,draw_line_1,weight_l,height_k,cr,beilv,svg_hist_size,stroke_size,type_h) 
                     cr.restore()    
             j = j + hist_size-1
