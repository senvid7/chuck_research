...
def draw_common(local_photo,max_color,sencond_color,third_color,fourth_color,cr,weight_l,height_k,beilv,svg_hist_size,stroke_size):
    ...
    draw_line = np.array(draw_line, dtype=np.int32)
    first_line = compute_long(draw_line,n,m)
    draw_line= shrink_polygon(draw_line,first_line//8,71)   
    rand_color,rand_second_color,out_third_color= find_outer_color(draw_line,n*beilv,m*beilv,sencond_color,stroke_size)
    svg_out_draw(rand_color,rand_second_color,out_third_color,weight_l,height_k,n*beilv,m*beilv,stroke_size,cr)
    long = compute_long(draw_line,n,m)//3
    draw_line_second = shrink_polygon(draw_line,long,51)

    if  compute_polygon_area(draw_line_second) > (n*m*beilv*beilv//20):
        short = compute_long(draw_line_second,n,m)//3
        draw_line_third = shrink_polygon(draw_line_second,short,51)        
        if draw_line_third.shape[0] > 2 and compute_polygon_area(draw_line_second) > compute_polygon_area(draw_line_third) > (n*m*beilv*beilv//16) :
            rand_color,rand_second_color,out_third_color = find_inner_color2(draw_line,draw_line_second,draw_line_third,n,m,max_color,stroke_size)
            ...
            rand_color,out_sencond_color = find_inner_color1(draw_line,draw_line_second,n*beilv,m*beilv,max_color,stroke_size)
            svg_draw(draw_line,rand_color,3,cr)
            svg_draw(draw_line_second,out_sencond_color,3,cr) 
    else:
        long = compute_long(draw_line,n,m)//5
        draw_line_second = shrink_polygon(draw_line,long,51)
        rand_color,out_sencond_color = find_inner_color1(draw_line,draw_line_second,n*beilv,m*beilv,max_color,stroke_size)
        svg_draw(draw_line,rand_color,3,cr)
        svg_draw(draw_line_second,out_sencond_color,3,cr)
...
def find_outer_color(draw_line,n,m,second_color,stroke_size):
    rang = 30
    flag = 0
    path = 0
    ...
    weight_2 = weight_2/weight_4 
    weight_3 = weight_3/weight_4
    while(flag == 0 ):
        if(path > 20):
            first_color,sencond_color,flag = color_cal2(weight_1,weight_4,second_color,rang)
            print(first_color,sencond_color,flag,weight_1,weight_4,second_color,"out")
        else:
            first_color,sencond_color,flag = color_cal(weight_1,weight_4,second_color,rang)
        if(flag != 0):
            if(path > 20):
                third_color,fourth_color,flag = color_cal2(weight_2,weight_3,sencond_color,rang)
            else:
                third_color,fourth_color,flag = color_cal(weight_2,weight_3,sencond_color,rang) 
        path = path +1    
    return first_color,third_color,fourth_color

def find_inner_color2(draw_line,draw_line_second,draw_line_third,n,m,second_color,stroke_size):
    rang = 30
    flag = 0
    path = 0
    ...
    while(flag == 0 ):
        if(path > 20):
            first_color,sencond_color,flag = color_cal2(weight_1,weight_2,second_color,rang)
            #print(first_color,sencond_color,flag,weight_1,weight_2,second_color,"inner2")
        else:
            first_color,sencond_color,flag = color_cal(weight_1,weight_2,second_color,rang)
        if(flag != 0):
            if(path > 20):
                third_color,fourth_color,flag = color_cal2(weight_3,weight_4,sencond_color,rang)
                print(first_color,sencond_color,flag,weight_3,weight_4,second_color)
            else:
                third_color,fourth_color,flag = color_cal(weight_3,weight_4,sencond_color,rang) 
        path = path +1
    return first_color,third_color,fourth_color

def color_cal(weight_1,weight_4,sencond_color,rang):
...
def color_cal2(weight_1,weight_2,sencond_color,rang):
    rand_color = np.zeros((3))
    out_sencond_color = np.zeros((3))
    flag = 1
...
def find_inner_color1(draw_line,draw_line_second,n,m,second_color,stroke_size):
    rang = 30
    flag = 0
    path = 0
    line_w_1 = compute_polygon_area(draw_line)
...
def svg_out_draw(rand_color,rand_second_color,out_third_color,weight_l,height_k,n,m,stroke_size,cr):  
    rand_color[:]=rand_color[:]/255
    rand_second_color[:]=rand_second_color[:]/255
    out_third_color[:]=out_third_color[:]/255
    ...
    cr.new_path()
    cr.move_to(stroke_size,stroke_size)
    cr.line_to(n-stroke_size,stroke_size)
    cr.line_to(n-stroke_size,m-stroke_size)
    cr.line_to(stroke_size, m-stroke_size)
    cr.close_path()
    cr.set_line_width(stroke_size*2)
    cr.stroke_preserve()
    cr.set_source_rgb(out_third_color[0], out_third_color[1],out_third_color[2])
    cr.new_path()
    cr.move_to(stroke_size*3,stroke_size*3)
    cr.line_to(n-stroke_size*3,stroke_size*3)
    cr.line_to(n-stroke_size*3,m-stroke_size*3)
    cr.line_to(stroke_size*3, m-stroke_size*3)
    cr.close_path()
    cr.stroke_preserve()
...
def compute_long(draw_line,n,m):
    center = centroid(draw_line)
    a = 0 
    for i in range(0,draw_line.shape[0]):
...
def draw_hist(probs,bins,local_photo,num_color,center,draw_line,weight_l,height_k,cr,beilv,svg_hist_size,stroke_size):
    probs_list = sorted(probs)
    for m in range (0,num_color):
    ...
    if draw_line_second.shape[0] < 3 or compute_polygon_area(draw_line) < (n*m*beilv*beilv//5):
        local_photo = draw_common(local_photo,max_color,third_color,sencond_color,fourth_color,cr,weight_l,height_k,beilv,svg_hist_size,stroke_size)                
    else:
        if m > n:
            weight_l = weight_l - svg_hist_size
        elif n > m:
    ...
    while i < (m-half_size):
        ...
        while j < (n-half_size):
             ...
             if (bins[4] - bins[0]) < color_size :
                 ...
                 cr.save()
                 local_photo1 = draw_common(local_photo,max_color,third_color,second_color,fourth_color,cr,weight_l,height_k,beilv,svg_hist_size,stroke_size)  
                 cr.restore()
             else:
                 ...
                 if draw_line is None :
                     ...
                     cr.save()
                     local_photo1 = draw_hist(probs,bins,local_photo_m,num_color,center,draw_line_1,weight_l,height_k,cr,beilv,svg_hist_size,stroke_size ) 
                     cr.restore()          
             j = j + hist_size
             height_k = height_k + svg_hist_size
        i = i + hist_size
